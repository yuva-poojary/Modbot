const config = {
  ...
  "token": "NjIwODc1OTY3NDkzMzA4NDMw.XYuWvg.dRnyXpBGDqZ_Aqe4jY9o37iGMHg",
  ...
  permLevels: [
    ...
    { level: 6,
      ...
      check: (message) => {
        try {
          if (message.member.hasPermission('MANAGE_CHANNELS')) return true;
        } catch (e) {
          return false;
        }
      }
    ...
    { level: 5,
      ...
      check: (message) => {
        try {
          if (message.member.hasPermission('MANAGE_ROLES_OR_PERMISSIONS')) return true;
        } catch (e) {
          return false;
        }
      }
